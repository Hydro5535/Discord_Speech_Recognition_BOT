...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("mapa")
        .setDescription("Informuje jaka mapa obecnie figuruje w normalnych meczach w grze Apex Legends"),
    async execute(client, interaction, distube) {
        let token = client.config.apex_secret;

        if (!token)
        {
            interaction.reply("Nie podano klucza api wymaganego dla tej komendy!");
        }
        else
        {
